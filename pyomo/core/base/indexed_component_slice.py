            ...
            while idx < self._slice._len:
                ...
                if _call[0] == IndexedComponent_slice.get_attribute:
                    ...
                    try:
                        ...
                        # E.g. NoneType has no attribute __getitem__
                        # This can happen if we try to call 
                        # `b[:].component(name)[i]`
                        # but the name is not valid (perhaps just for some
                        # particular index of the slice).
                        #
