...
class TestExternalGreyBoxBlock(unittest.TestCase):
    ...
    @unittest.skipUnless(cyipopt_available, "cyipopt is not available")
    def test_optimize_dynamic_references_ipopt(self):
        """
        Itentical to the above test, but now Ipopt is used as a solver
        for the implicit function instead of CyIpopt, which is the default.
        """
        # Create the "external model"
        m = make_dynamic_model()
        time = m.time
