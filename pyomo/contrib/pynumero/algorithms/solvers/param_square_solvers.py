...
class CompressedSccSolver(SquareDecompositionSolver):

    def partition_system(self, variables, equations):
        imat = get_structural_incidence_matrix(variables, equations)
        M, N = imat.shape
        ...
        eqn_partition = [
            [equations[i] for i in subset] for subset in partition
        ]
        var_partition = [
            [variables[matching[i]] for i in subset]
            for subset in partition
        ]
        subsystems = zip(var_partition, eqn_partition)
        return subsystems
